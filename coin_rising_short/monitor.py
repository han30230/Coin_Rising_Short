...
def get_futures_gainers_and_top_movers(
    ...
    for t in data:
        ...
        if symbol not in symbols.TRADING_SYMBOLS:
            continue
        try:
            change_pct = Decimal(str(t["priceChangePercent"]))
            turnover_24h = Decimal(str(t["quoteVolume"]))
            last_price = Decimal(str(t["lastPrice"]))
...
def monitor_loop() -> None:
    ...
    logger.info(
        "Binance 선물 급등 감시 (+%s%%, ST감시=%s개, SuperTrend 진입=%s, 재진입=%s, 최대포지션=%s)...",
